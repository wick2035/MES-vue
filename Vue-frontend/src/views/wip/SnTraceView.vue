  ...
  <div class="space-y-4">
    ...
    <Card>
      <CardContent class="p-6">
        ...
        <div v-else-if="records.length === 0" class="flex flex-col items-center gap-2 py-10 text-muted-foreground">
          <Boxes class="h-8 w-8" />
              ...
              <span
                :class="
                  cn(
                    ...
                    r.status === 'OK' ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive',
                  )
                "
              >{{ r.status }}</span>
            </div>
            <div class="mt-0.5 text-xs text-muted-foreground">{{ r.createTime }}</div>
            <div v-if="r.remark" class="mt-0.5 text-xs text-muted-foreground">备注：{{ r.remark }}</div>
          </li>
        </ol>
      </CardContent>
    </Card>
