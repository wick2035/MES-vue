...
public class SpWorkflowDefinitionController extends WorkflowBaseController {
    ...
    public String addOrUpdateUI(Model model, SpWorkflowForm record) {
        ...
        applyDefaultFormValues(result);
        model.addAttribute("result", result);
        return "workflow/definition/addOrUpdate";
    }
    ...
    public Result delete(SpWorkflowForm record) {
        if (record == null || StringUtils.isBlank(record.getId())) {
            return Result.failure("请选择要删除的流程定义表单");
        }
        formService.removeById(record.getId());
        return Result.success();
    }

    private void applyDefaultFormValues(SpWorkflowForm result) {
