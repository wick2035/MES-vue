...
const emit = defineEmits<{
  (e: 'update:checked', checked: string[]): void
}>()
...
function toggleCheck(id: string, val: boolean | 'indeterminate') {
  ...
  if (val === true) next.add(id)
  else next.delete(id)
  emit('update:checked', Array.from(next))
}
</script>
  ...
  <ul class="space-y-0.5">
    <li v-for="node in nodes" :key="node.id">
      <div class="flex items-center gap-1.5 rounded px-1 py-1 hover:bg-accent">
        <button v-if="node.children?.length" class="text-muted-foreground" @click="toggle(node.id)">
          <ChevronRight
      ...
      <div v-if="node.children?.length && expanded.has(node.id)" class="ml-5 border-l pl-2">
        <SpTree
          :nodes="node.children"
          :checkable="checkable"
          :checked="checked"
